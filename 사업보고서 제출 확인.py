...
## 매서드 정의
# 제출한 공시 보고서 조회하는 매서드
def collect_raw_anuual_report_list(df):
...
rep_list = list(res_df["corp_code"].unique())
...
df['사업보고서 제출여부2'] = df['고유번호'].apply(is_company_anuual_repot)
df.to_excel('사업보고서 추가2.xlsx',index=False)
