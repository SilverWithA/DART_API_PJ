...
class API_info_finder:
    ...
    # 특정 공시보고서 제출 목록 불러오기
    def 공시보고서_접수번호목록_가져오기(self, corpcode_list, start_date, end_date, pblntf_ty, pblntf_detail_ty):
    ...
    # 특정 공시보고서 제출 목록 불러오기
    def 공시보고서_접수번호목록_가져오기_디테일버전(self, corpcode_list, start_date, end_date, pblntf_detail_ty):
        """고유번호를 기반으로 특정공시보고서 리스트를 반환하는 매서드"""
    ...
    def 공시보고서_접수번호목록_가져오기_no고유번호(self, start_date, end_date, pblntf_ty, pblntf_detail_ty):
        """(기간은 3개월 미만으로 지정해야함 주의)
        고유번호 없이 공시보고서 리스트를 반환하는 매서드"""

        res_df = pd.DataFrame(columns=['corp_code', 'corp_name', 'stock_code','corp_cls', 'report_nm', 'rcept_no', 'flr_nm', 'rcept_dt', 'rm'])
