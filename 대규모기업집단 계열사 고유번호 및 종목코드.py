...
df = pd.read_excel('대규모기업집단 연도별 계열사 및 경영성과.xlsx', sheet_name='중간확인')

# KRX 전종목 기본정보 로드(디렉토리에 미리 저장 필요)
krx_df = pd.read_excel('KRX  전종목 기본정보_20250306.xlsx',sheet_name='Sheet1')

# 다트기준 회사명으로 고유번호 서치
df['고유번호'] = df['다트기준 소속회사명'].apply(find_corp_num)
...
for country in root.iter("list"):
    if (country.findtext("corp_name") ==
            '스튜디오툰(StudioToonCorp.)'):
        print(country.findtext("corp_code"))




# 매핑 안된 계열사명 확인하기
df_na = df.loc[df['고유번호'].isna()]

# 고유번호로 종목코드 서치(다트기준 종목코드)
df['종목코드'] = df['고유번호'].apply(find_stock_code)CCC
