...
def 기업개황_excel_업데이트():
    from app.model.Info_collecter import recent_overview_updater

    overview_udt = recent_overview_updater()

    overview_udt.dowmload_recent_cmp_overview()
    df = overview_udt.convert_overview_to_df()
    df.to_excel('DART 기업개황 전체_20250602.xlsx', index=False)
...
def 네이버_보도자료_아카이브():
    ...
    news_df = pd.read_excel('raw 네이버뉴스 크롤링.xlsx')

    news_html = newscontroller.데이터프레임_html형식_변환기(news_df)

    with open("뉴스 크롤링 html_20250522.txt","w") as f:
        f.write(news_html)
